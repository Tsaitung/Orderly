...
const SupplierStatistics = React.forwardRef<HTMLDivElement, SupplierStatisticsProps>(
  ({ statistics, isLoading = false, error = null, onRefresh, className, ...props }, ref) => {
    // Helper functions
    ...
    const ErrorState = () => (
      <Card className="p-8 text-center">
        ...
        <p className="mb-4 text-gray-600">{error || '無法載入統計資料'}</p>
        {onRefresh && (
      ...
      <div ref={ref} className={cn('spacing-normal', className)} {...props}>
          ...
          <div>
            ...
            {period && (
              <p className="mt-1 text-sm text-gray-600">
                統計期間：{new Date(period.start).toLocaleDateString()} -{' '}
          ...
          {computedCards.map((card, index) => {
            ...
            return (
              <Card
                ...
                variant="supplier"
                padding="sm"
                className="border-l-4 border-supplier-500"
              >
                <div className="flex items-center justify-between">
                  <div>
        ...
        {topSuppliers.length > 0 && (
          <Card variant="glass-supplier" padding="md">
            <h3 className="mb-4 text-lg font-semibold text-gray-900">表現優異供應商 TOP 5</h3>
                ...
                <div key={supplier.id} className="flex items-center justify-between py-2">
                  <div className="flex items-center space-x-3">
                    <Badge
                      ...
                      className={cn(
                        'flex h-6 w-6 items-center justify-center p-0 text-xs',
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500">{supplier.orderCount} 筆訂單</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 gap-3 text-center md:grid-cols-3">
            <div>
              ...
              <div
                className={cn(
                  ...
                  averageFulfillmentRate >= 90
                    ? 'text-green-600'
                    : averageFulfillmentRate >= 80
                      ? 'text-yellow-600'
                      : 'text-red-600'
                )}
              >
            ...
            <div>
              ...
              <div className="text-lg font-semibold text-gray-900">
                {computed.averageProductsPerSupplier > 50
                  ? '深度合作'
                  : computed.averageProductsPerSupplier > 20
                    ? '一般合作'
                    : '初期合作'}
              </div>
            </div>
            ...
            <div>
              ...
              <div
                className={cn(
                  'text-lg font-semibold',
                  computed.supplierUtilizationRate >= 80
                    ? 'text-green-600'
                    : computed.supplierUtilizationRate >= 60
                      ? 'text-yellow-600'
                      : 'text-red-600'
                )}
              >
