...
export default function ReconciliationStatus() {
  ...
  const handleProcessItem = React.useCallback(
    (itemId: string) => {
      setData(prev =>
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">機器學習驅動的自動對帳系統即時狀態</p>
        </div>
        <Button
          variant="outline"
          size="sm"
                ...
                <div className="flex min-w-0 flex-1 items-center space-x-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center space-x-4">
                      <span className="text-xs text-gray-500">{getTypeText(item.type)}</span>
                      <span
                        className={cn('text-xs font-medium', getConfidenceColor(item.confidence))}
                      >
                    ...
                    {item.discrepancy && (
                      <div className="mt-1 text-xs text-red-600">
                        {item.discrepancy.field}差異: {item.discrepancy.expected} →{' '}
                        {item.discrepancy.actual}
                      </div>
                    )}
                  </div>
                </div>
