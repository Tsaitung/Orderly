...
export default function ActivityIndicator({
  ...
  const renderTrend = () => {
    ...
    const trendIcon =
      activity.trend === 'up' ? TrendingUp : activity.trend === 'down' ? TrendingDown : Minus
    ...
    return (
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          <span className={cn(sizeConfig.score, 'font-medium text-gray-700')}>活躍度</span>
          <span className={cn(sizeConfig.score, 'font-bold', config.textColor)}>
            {activity.score}
          </span>
        </div>
        <div className={cn('w-full rounded-full bg-gray-200', sizeConfig.progress)}>
          <div
            className={cn(
              config.color,
              sizeConfig.progress,
  ...
  return (
    <div className={cn('space-y-2', className)}>
      ...
      <div className={cn('space-y-1', sizeConfig.container, 'text-gray-600')}>
        ...
        <div className="flex justify-between">
          <span>月營收:</span>
          <span className="font-medium">NT$ {(activity.monthlyRevenue / 1000).toFixed(0)}K</span>
        </div>

        <div className="flex justify-between">
          <span>最後訂單:</span>
          <span className="font-medium">{formatLastOrder(activity.lastOrderDate)}</span>
        </div>
      </div>
    </div>
  )
...
export function ActivityBadge({ level, score, size = 'sm', className }: ActivityBadgeProps) {
...
export function ActivityDot({ level, size = 'sm', className, title }: ActivityDotProps) {
  const config = ACTIVITY_CONFIG[level]
  const sizeConfig = SIZE_CONFIG[size]

  return (
    <div
      className={cn('rounded-full', sizeConfig.indicator, config.color, className)}
