        ...
        actions: {
          ...
          updateProfile: async (organizationId: string, data: SupplierProfileUpdateRequest) => {
            ...
            try {
              ...
              set(draft => {
                ...
                draft.profileCache.set(organizationId, {
                  data: updatedProfile,
                  timestamp: Date.now(),
                })
                draft.loading.profile = false
          ...
          createProfile: async (data: any) => {
            ...
            try {
              ...
              set(draft => {
                draft.profile = profile
                draft.profileCache.set(profile.organization_id, {
                  data: profile,
                  timestamp: Date.now(),
                })
                draft.loading.profile = false
          ...
          refreshMetrics: async (organizationId: string) => {
            ...
            await Promise.all([fetchMetrics(organizationId), fetchDashboard(organizationId)])
          },
          ...
          updateCustomer: async (
            ...
            try {
              const updatedCustomer = await supplierService.updateCustomer(
                organizationId,
                customerId,
                data
              )
          ...
          updateOnboardingStep: async (organizationId: string, stepName: string, data: any) => {
            try {
              const updatedOnboarding = await supplierService.updateOnboardingStep(
                organizationId,
                stepName,
                data
              )
          ...
          completeOnboardingStep: async (organizationId: string, stepName: string) => {
            try {
              const updatedOnboarding = await supplierService.completeOnboardingStep(
                organizationId,
                stepName
              )
          ...
          refreshAll: async (organizationId: string) => {
            const { fetchProfile, fetchDashboard, fetchMetrics, fetchCustomers, fetchOnboarding } =
              get().actions
...
export const useSupplierDashboard = () =>
  useSupplierStore(state => ({ dashboard: state.dashboard, metrics: state.metrics }))
export const useSupplierCustomers = () =>
  useSupplierStore(state => ({ customers: state.customers, pagination: state.customersPagination }))
