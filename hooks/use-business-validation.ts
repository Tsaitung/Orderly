 ...
 */
export function useTaxIdValidation(taxId: string, options: UseBusinessValidationOptions = {}) {
  const { debounceMs = 800, validateOnMount = false, cacheResults = true } = options
    ...
    async (id: string) => {
      if (!id || id.length !== 8) {
        setState({
          ...
          result: null,
          error: id
            ? VALIDATION_MESSAGES.TAX_ID.INVALID_FORMAT
            : VALIDATION_MESSAGES.TAX_ID.REQUIRED,
        })
        return
      }
...
export function useBusinessIdentifierValidation(
  ...
  const taxIdValidation = useTaxIdValidation(businessType === 'company' ? identifier : '', {
    ...options,
    validateOnMount: businessType === 'company' && options.validateOnMount,
  })
