...
export function BusinessIdentifierInput({
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor={`business-identifier-${businessType}`} className="flex items-center space-x-2">
        <span>{getLabel()}</span>
      ...
      {field.message && (
        <div className={cn(
          ...
          {field.status === 'validating' && <Loader2 className="w-3 h-3 animate-spin" />}
          {field.status === 'success' && <CheckCircle className="w-3 h-3" />}
          {field.status === 'error' && <AlertCircle className="w-3 h-3" />}
      ...
      {field.status === 'error' && 
       field.validation.error?.includes('API') && (
        <Button
          type="button"
          variant="outline"
          size="sm"
