...
interface RestaurantOrder {
  ...
  items: OrderItem[]
  totalAmount: number
  status: 'draft' | 'pending' | 'confirmed' | 'preparing' | 'shipped' | 'delivered' | 'accepted' | 'completed' | 'cancelled'
...
const mockOrders: RestaurantOrder[] = [
  {
    ...
    items: [
      { id: '1-1', name: '有機高麗菜', quantity: 10, unit: '公斤', unitPrice: 45, totalPrice: 450, category: '蔬菜類' },
      { id: '1-2', name: '新鮮蘿蔔', quantity: 8, unit: '公斤', unitPrice: 30, totalPrice: 240, category: '蔬菜類' }
    ],
    totalAmount: 690,
    status: 'pending',
    priority: 'medium',
  ...
  },
  {
    ...
    items: [
      { id: '2-1', name: 'A級牛肉', quantity: 5, unit: '公斤', unitPrice: 800, totalPrice: 4000, category: '肉品類' },
      { id: '2-2', name: '新鮮豬肉', quantity: 3, unit: '公斤', unitPrice: 300, totalPrice: 900, category: '肉品類' }
    ],
    totalAmount: 4900,
    status: 'confirmed',
    priority: 'high',
  ...
  },
  {
    ...
    items: [
      { id: '3-1', name: '新鮮鮭魚', quantity: 2, unit: '公斤', unitPrice: 1200, totalPrice: 2400, category: '海鮮類' },
      { id: '3-2', name: '活蝦', quantity: 1, unit: '公斤', unitPrice: 800, totalPrice: 800, category: '海鮮類' }
    ],
    totalAmount: 3200,
    status: 'delivered',
    priority: 'urgent',
  ...
  },
  {
    ...
    items: [
      { id: '4-1', name: '特級醬油', quantity: 6, unit: '瓶', unitPrice: 120, totalPrice: 720, category: '調味料' },
      { id: '4-2', name: '香油', quantity: 4, unit: '瓶', unitPrice: 200, totalPrice: 800, category: '調味料' }
    ],
    totalAmount: 1520,
    status: 'completed',
    priority: 'low',
...
export default function RestaurantOrderManagement() {
  ...
  const handleSort = React.useCallback((field: keyof RestaurantOrder) => {
    if (sortField === field) {
  ...
  const handleViewDetail = React.useCallback((order: RestaurantOrder) => {
  ...
  const getPaymentStatusBadge = (status: RestaurantOrder['paymentStatus']) => {
    switch (status) {
      case 'paid': return <Badge variant="subtle" colorScheme="green" size="sm">已付款</Badge>
      case 'pending': return <Badge variant="subtle" colorScheme="yellow" size="sm">待付款</Badge>
      case 'overdue': return <Badge variant="subtle" colorScheme="red" size="sm">逾期</Badge>
    }
  }

  const getReconciliationBadge = (status: RestaurantOrder['reconciliationStatus']) => {
    switch (status) {
      case 'pending': return <Badge variant="outline" colorScheme="gray" size="sm">待對帳</Badge>
      case 'matched': return <Badge variant="solid" colorScheme="green" size="sm">已匹配</Badge>
      case 'discrepancy': return <Badge variant="outline" colorScheme="red" size="sm">有差異</Badge>
      case 'resolved': return <Badge variant="solid" colorScheme="green" size="sm">已解決</Badge>
    }
  }
