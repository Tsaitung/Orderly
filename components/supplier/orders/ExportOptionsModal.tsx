  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div>
          <h3 className="mb-3 font-medium">選擇匯出格式</h3>
              ...
              return (
                ...
                >
                  <div className="flex items-start space-x-3">
                    <Icon
                      className={`mt-1 h-6 w-6 ${isSelected ? 'text-blue-600' : 'text-gray-400'}`}
                    />
                    <div className="flex-1">
                      ...
                      <p
                        className={`mt-2 text-xs ${isSelected ? 'text-blue-600' : 'text-gray-500'}`}
                      >
            ...
            <div className="mt-3 rounded-md border border-yellow-200 bg-yellow-50 p-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-yellow-800">
                  資料將被截斷：共 {totalOrders.toLocaleString()} 筆，將匯出前{' '}
                  {formatConfig.maxRows.toLocaleString()} 筆
                </span>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {/* Date Range */}
          <div>
            <h3 className="mb-3 font-medium">日期範圍（選填）</h3>
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-sm text-gray-700">開始日期</label>
                <input
                  type="date"
              ...
              <div>
                <label className="mb-1 block text-sm text-gray-700">結束日期</label>
                <input
                  type="date"
                  value={dateRange.to}
          ...
          {/* Export Options */}
          <div>
            <h3 className="mb-3 font-medium">匯出選項</h3>
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                <Checkbox checked={includeItems} onChange={setIncludeItems} />
                <label className="text-sm text-gray-700">包含商品明細</label>
              </div>

              <div className="flex items-center space-x-2">
                <Checkbox checked={includeNotes} onChange={setIncludeNotes} />
                <label className="text-sm text-gray-700">包含備註說明</label>
              </div>

              <div className="flex items-center space-x-2">
                <Checkbox checked={summaryStats} onChange={setSummaryStats} />
                <label className="text-sm text-gray-700">包含統計摘要</label>
              </div>

              <div>
                <label className="mb-1 block text-sm text-gray-700">分組方式</label>
                <select
                  value={groupBy}
        ...
        <div className="rounded-md border border-gray-200 bg-gray-50 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {Object.keys(currentFilters).filter(
              k => currentFilters[k] && k !== 'page' && k !== 'page_size'
            ).length === 0 && <span className="text-sm text-gray-500">無篩選條件</span>}
          </div>
