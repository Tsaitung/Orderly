...
import { Save, X, Loader, Package, AlertCircle, Eye, EyeOff } from 'lucide-react'
...
export default function ProductEditModal({
  ...
  const validateForm = (): boolean => {
    ...
    if (
      formData.max_order_quantity !== undefined &&
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'basic' && (
          <div className="space-y-6">
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">產品名稱</label>
              <Input
                value={formData.name || ''}
            ...
            <div>
              <label className="mb-3 block text-sm font-medium text-gray-700">產品狀態</label>
            ...
            <div>
              <div className="mb-2 flex items-center space-x-2">
                ...
                />
                <label className="text-sm font-medium text-gray-700">設為精選商品</label>
              </div>
              <p className="text-xs text-gray-500">精選商品會在產品列表中優先顯示</p>
            </div>
        ...
        {activeTab === 'pricing' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">最小訂購量</label>
                <Input
                  type="number"
                  value={formData.min_order_quantity || ''}
                  onChange={e =>
                    handleFieldChange('min_order_quantity', parseInt(e.target.value) || 1)
                  }
                  min="1"
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">最大訂購量</label>
                <Input
                  type="number"
                ...
                <p className="mt-1 text-xs text-gray-500">
                  ...
                  {product.max_order_quantity
                    ? `${product.max_order_quantity} ${product.unit_type}`
                    : '不限制'}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={e =>
                  handleFieldChange('quality_info.storage_instructions', e.target.value)
                }
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">過敏原資訊</label>
              <Input
                value={formData.quality_info?.allergen_info?.join(', ') || ''}
                onChange={e =>
                  handleFieldChange(
                    'quality_info.allergen_info',
                    e.target.value
                      .split(',')
                      .map(s => s.trim())
                      .filter(Boolean)
              ...
              />
              <p className="mt-1 text-xs text-gray-500">多個過敏原請用逗號分隔</p>
            </div>
          </div>
        )}
