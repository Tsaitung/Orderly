...
export default function CustomerFilters({ searchParams }: CustomerFiltersProps) {
  ...
  const getActiveFilterCount = () => {
    let count = Object.values(activeFilters).filter(value => value !== 'all' && value !== 'name-asc').length
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-6 gap-6">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {customerSegments.map((segment) => (
                <label key={segment.id} className="flex items-center space-x-2 cursor-pointer">
                  ...
                  <Badge variant="outline" size="sm">{segment.count}</Badge>
                </label>
              ))}
            </div>
          </div>
          ...
          {/* 客戶類型 */}
          <div>
            <h4 className="text-sm font-medium text-gray-700 mb-3">客戶類型</h4>
          ...
          {/* 消費金額 */}
          <div>
            <h4 className="text-sm font-medium text-gray-700 mb-3">月消費額</h4>
            <div className="space-y-2">
          ...
          {/* 活躍狀態 */}
          <div>
            <h4 className="text-sm font-medium text-gray-700 mb-3">活躍狀態</h4>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              {satisfactionLevels.map((level) => (
                <label key={level.id} className="flex items-center space-x-2 cursor-pointer">
                  ...
                  <Badge variant="outline" size="sm">{level.count}</Badge>
                </label>
              ))}
            </div>
          </div>
