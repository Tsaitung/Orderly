...
import {
  SupplierDashboardSkeleton,
  SupplierMetricsGridSkeleton,
} from './shared/SupplierLoadingStates'
...
function SupplierMetricCard({
  ...
  const getChangeColor = () => {
    ...
    const isGoodChange =
      title.includes('營收') || title.includes('訂單') || title.includes('評分')
  ...
  return (
    <Card className="relative overflow-hidden transition-all hover:shadow-md">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        ...
        <div className={cn('rounded-lg bg-opacity-10 p-2', getStatusColor())}>{icon}</div>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
              ...
              <Progress
                ...
                variant={
                  getTargetProgress()! >= 100
                    ? 'success'
                    : getTargetProgress()! >= 80
                      ? 'default'
                      : getTargetProgress()! >= 60
                        ? 'warning'
                        : 'destructive'
                }
              />
          ...
          {status !== 'good' && (
            <Badge
              ...
              className="mt-2"
            >
              {status === 'excellent' ? '優秀' : status === 'warning' ? '需關注' : '需改善'}
            </Badge>
          )}
        </div>
      </CardContent>
...
function SupplierOverviewContent({ organizationId }: SupplierOverviewContentProps) {
  const { dashboard, metrics, loading, error, refreshMetrics, autoRefresh, setAutoRefresh } =
    ...
    return [
      {
        ...
        change: {
          value: getChangePercentage(
            ...
            metrics.today_orders - Math.floor(Math.random() * 3)
          ),
        ...
        status:
          metrics.today_orders >= 10
            ? ('excellent' as const)
            : metrics.today_orders >= 5
              ? ('good' as const)
              : ('warning' as const),
      ...
      },
      {
        ...
        status:
          metrics.customer_satisfaction_rate >= 4.5 ? ('excellent' as const) : ('good' as const),
      ...
      },
      {
        ...
        status:
          metrics.pending_orders + metrics.in_progress_orders <= 5
            ? ('excellent' as const)
            : ('good' as const),
      ...
      },
      {
        ...
        change: {
          value: getChangePercentage(
            ...
            Math.max(1, metrics.active_customers - 2)
          ),
      ...
      },
      {
        ...
        change: {
          value: getChangePercentage(
            ...
            Math.max(1000, metrics.week_revenue * 0.8)
          ),
          type: 'increase' as const,
          period: '上週',
        },
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                {achievements.map((achievement, index) => (
                  <div
                    key={index}
                    className="flex items-center space-x-3 rounded-lg border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-4"
                  >
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="space-y-3">
                  ...
                  <Progress
                    value={
                      metrics.week_orders > 0
                        ? (metrics.completed_orders_today / metrics.week_orders) * 100
                        : 0
                    }
                    className="h-2"
                    variant="success"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-green-600">
                      {metrics.customer_satisfaction_rate.toFixed(1)}★
                    </span>
                  </div>
                  <Progress
                  ...
                  />
                  <p className="text-xs text-gray-500">
                    {metrics.customer_satisfaction_rate >= 4.5
                      ? '超越 4.5★ 目標，表現優異'
                      : '持續努力提升服務品質'}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  />
                  <p className="text-xs text-gray-500">
                    {metrics.on_time_delivery_rate >= 95
                      ? '超越 95% 目標，表現卓越'
                      : '努力提升交付準時率'}
                  </p>
                </div>
