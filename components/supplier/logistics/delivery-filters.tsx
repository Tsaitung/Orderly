...
import { Search, Filter, Calendar, MapPin, Truck, Clock, Package, User } from 'lucide-react'
...
export default function DeliveryFilters({ searchParams }: DeliveryFiltersProps) {
  ...
  const getActiveFilterCount = () => {
    let count = Object.values(activeFilters).filter(
      value => value !== 'all' && value !== 'created-desc'
    ).length
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 xl:grid-cols-6">
          ...
          {/* 優先級 */}
          <div>
            <h4 className="mb-3 text-sm font-medium text-gray-700">優先級</h4>
            <div className="space-y-2">
              {priorityLevels.map(priority => (
                <label key={priority.id} className="flex cursor-pointer items-center space-x-2">
                  <input
                    type="radio"
                    name="priority"
                    value={priority.id}
          ...
          <div>
            ...
            <div className="max-h-48 space-y-2 overflow-y-auto">
              {drivers.map(driver => (
                <label key={driver.id} className="flex cursor-pointer items-center space-x-2">
                  <input
                    type="radio"
                    name="driver"
                    value={driver.id}
      ...
      {getActiveFilterCount() > 0 && (
        <Card className="border-blue-200 bg-blue-50 p-4">
          <div className="flex items-center justify-between">
            <div className="flex flex-wrap items-center space-x-2 text-sm text-blue-700">
              <span>當前篩選:</span>
            ...
            <span className="text-sm font-medium text-blue-700">顯示 156 筆配送中的 89 筆</span>
          </div>
        </Card>
      )}
    </div>
