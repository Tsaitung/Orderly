...
export function UserDetail({ userId }: UserDetailProps) {
  ...
  const [activeTab, setActiveTab] = React.useState<'overview' | 'permissions' | 'organizations' | 'activity' | 'settings'>('overview')
  ...
  const getStatusBadge = (status: UserDetailData['status']) => {
    switch (status) {
      case 'active':
        return <Badge className="bg-green-100 text-green-800"><CheckCircle className="h-3 w-3 mr-1" />活躍</Badge>
      case 'inactive':
        return <Badge className="bg-red-100 text-red-800"><XCircle className="h-3 w-3 mr-1" />已停用</Badge>
      case 'pending':
        return <Badge className="bg-yellow-100 text-yellow-800"><Clock className="h-3 w-3 mr-1" />待啟用</Badge>
      default:
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'permissions' && (
        <PermissionManager userId={userId} />
      )}

      {/* 組織架構標籤 */}
      {activeTab === 'organizations' && (
        <OrganizationTree userId={userId} />
      )}
      ...
      {activeTab === 'settings' && (
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              <h4 className="font-medium text-gray-900 mb-3">地區設定</h4>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
