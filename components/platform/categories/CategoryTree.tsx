    ...
    <div className="select-none">
      ...
      >
        <div className="flex items-center space-x-2 flex-1 min-w-0">
          ...
          {/* Category Icon */}
          <FolderTree className="h-4 w-4 text-gray-500 flex-shrink-0" />
...
export function CategoryTree({ 
  ...
  const buildTree = (cats: Category[]): Category[] => {
    ...
    cats.forEach(cat => {
      if (cat.level === 2) {
        ...
        const parent = possibleParents.find(p => {
          ...
          if (p.code === 'VEGG' && ['LETC', 'ROOT', 'GOUR', 'FLFV', 'BEAN', 'MUSH', 'ARSS', 'HERB', 'STEM', 'HVGV', 'SPRO', 'SEAW', 'CUTV', 'PRVG', 'VMSC'].includes(cat.code)) {
            return true
          }
          if (p.code === 'FRUT' && ['BERR', 'PSTF', 'TROP', 'MELN', 'FRFR', 'PRFR', 'FRMS'].includes(cat.code)) {
            return true
          }
          if (p.code === 'BEEF' && ['CHUC', 'RIBE', 'BLON', 'BRIS', 'FLNK', 'ROUN', 'BGRD', 'BOFF', 'PRBF', 'BFMS'].includes(cat.code)) {
            return true
          }
          if (p.code === 'PORK' && ['PHED', 'BUTT', 'PLON', 'BELI', 'FRNT', 'HLEG', 'TROT', 'PGRD', 'POFF', 'PRPK', 'PKMS'].includes(cat.code)) {
            return true
          }
          if (p.code === 'CHKN' && ['WHOL', 'BRST', 'THGH', 'WING', 'FEET', 'COFF', 'CGRD', 'PRCK', 'CKMS'].includes(cat.code)) {
            return true
          }
          if (p.code === 'OTME' && ['LAMB', 'DUCK', 'GOOS', 'PROT', 'OTMS'].includes(cat.code)) {
            return true
          }
          if (p.code === 'SEAF' && ['FISH', 'CRUS', 'SHEL', 'CEPH', 'PRSF', 'DRSF', 'SFMS'].includes(cat.code)) {
            return true
          }
          if (p.code === 'DETF' && ['EGGS', 'MILK', 'DAIR', 'SOYP', 'PRDT', 'DTMS'].includes(cat.code)) {
            return true
          }
    ...
    return cats.filter(cat => cat.level === 1).map(cat => categoryMap.get(cat.id)!).filter(Boolean)
  }
