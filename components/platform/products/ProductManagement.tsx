...
import { Package, TrendingUp, Star, Search, Building2, Loader2, AlertCircle, ShoppingCart, Calendar, MapPin, ArrowLeft } from 'lucide-react'
...
export function SKUManagement() {
  ...
  // 視圖狀態
  const [currentView, setCurrentView] = React.useState<'search' | 'supplier-comparison' | 'batch-management'>('search')
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">所有商品規格總數</p>
                {loading ? (
                  <div className="flex items-center space-x-2">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">可供訂購的商品規格</p>
                {loading ? (
                  <div className="flex items-center space-x-2">
                    <Loader2 className="h-4 w-4 animate-spin" />
                    <span className="text-sm text-gray-500">載入中...</span>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">提供商品的供應商數量</p>
                {loading ? (
                  <div className="flex items-center space-x-2">
                    <Loader2 className="h-4 w-4 animate-spin" />
                    <span className="text-sm text-gray-500">載入中...</span>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">點擊管理批次</p>
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <select 
                  ...
                  onChange={(e) => setFilters(prev => ({ ...prev, processing_method: e.target.value }))}
                  className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFilters(prev => ({ ...prev, min_price: parseFloat(e.target.value) || undefined }))}
                  ...
                  <Input
                    ...
                    onChange={(e) => setFilters(prev => ({ ...prev, max_price: parseFloat(e.target.value) || undefined }))}
                    className="w-20 text-sm"
                  />
                </div>
                <Button
                ...
                return (
                  <div className="space-y-2">
                    {displayProducts.map((product) => (
                      <div key={product.id} className="border rounded-lg p-2 hover:bg-gray-50 transition-colors">
                        <div className="space-y-1.5">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-3 flex-1">
                                  ...
                                  <span className="font-mono bg-gray-100 px-1.5 py-0.5 rounded text-xs">代碼: {product.code}</span>
                                  <span className="text-xs text-gray-600">包裝: {product.packageType || '散裝'}</span>
                                  {product.variant && Object.entries(product.variant).map(([key, value]) => (
                                    <span key={key} className="text-xs text-gray-600">{key}: {value as string}</span>
                                  ))}
                                </div>
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Badge variant={product.isPublic ? "outline" : "secondary"} className="text-xs">
                          ...
                          <div className="flex justify-end space-x-2 pt-1 border-t">
                            <button className="px-2 py-1 text-xs bg-white border border-gray-300 rounded hover:bg-gray-50 transition-colors">
                              查看詳情
                            </button>
                            <button 
