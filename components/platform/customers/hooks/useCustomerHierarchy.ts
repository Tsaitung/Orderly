...
interface UseCustomerHierarchyReturn {
  ...
  searchQuery: string;
  searchResults: Array<{ entity: HierarchyNode; score: number; matchType: string; breadcrumb: string[] }>;
...
export function useCustomerHierarchy(options: UseCustomerHierarchyOptions = {}): UseCustomerHierarchyReturn {
  ...
  const findNode = useCallback((nodeId: UUID): HierarchyNode | null => {
    return customerHierarchyService.findNodeInTree(state.tree, nodeId);
  }, [state.tree]);

  const getNodePath = useCallback((nodeId: UUID): HierarchyNode[] => {
    return customerHierarchyService.getNodePath(state.tree, nodeId);
  }, [state.tree]);

  const getNodeChildren = useCallback((nodeId: UUID): HierarchyNode[] => {
