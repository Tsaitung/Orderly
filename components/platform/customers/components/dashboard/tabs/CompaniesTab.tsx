    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <h3 className="mb-2 text-lg font-medium text-gray-900">沒有找到公司資料</h3>
              ...
              <tr>
                <th className="px-6 py-3 text-left">
                  <button
                    ...
                    className="flex items-center space-x-1 text-xs font-medium uppercase tracking-wider text-gray-500 hover:text-gray-700"
                  >
                    <span>公司名稱</span>
                ...
                <tr key={company.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-2">
                      <Button variant="ghost" size="sm" onClick={() => onCompanySelect(company)}>
...
export default function CompaniesTab({
  ...
  const filteredCompanies = useMemo(() => {
    ...
    if (filters.activityLevels.length < 4) {
      result = result.filter(
        company => company.activity && filters.activityLevels.includes(company.activity.level)
      )
    }
    ...
    if (filters.minRevenue !== undefined) {
      result = result.filter(
        company => company.metrics && company.metrics.monthlyRevenue >= filters.minRevenue!
      )
    }
    if (filters.maxRevenue !== undefined) {
      result = result.filter(
        company => company.metrics && company.metrics.monthlyRevenue <= filters.maxRevenue!
      )
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm text-gray-500">顯示 {filteredCompanies.length} 間公司</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Card variant="filled" colorScheme="primary">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-primary-900">{filteredCompanies.length}</div>
        ...
        <Card variant="filled" colorScheme="platform">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-platform-900">
              NT${' '}
              {Math.round(
                filteredCompanies.reduce((sum, c) => sum + (c.metrics?.monthlyRevenue || 0), 0) /
                  1000
              )}
              K
            </div>
      ...
      {/* Companies Table */}
      <CompaniesTable companies={filteredCompanies} onCompanySelect={handleCompanySelect} />
    </div>
  )
}
