    ...
    >
      ...
      <CardContent className="pt-0">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <div className="text-xs text-gray-500 mb-1">月訂單</div>
            <div className="text-lg font-bold text-gray-900">
              {activity.orderFrequency}
            </div>
          </div>
          
          <div>
            <div className="text-xs text-gray-500 mb-1">平均訂單</div>
    ...
    return (
      <Card className="text-center py-12">
        <CardContent>
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">沒有找到客戶集團</h3>
...
export default function GroupsTab({
  ...
  const enhancedGroups = useMemo<EnhancedGroupNode[]>(() => {
    const groups = searchQuery && searchResults.length > 0
  ...
  const filteredAndSortedGroups = useMemo(() => {
    ...
    if (filters.activityLevels.length < 4) {
      result = result.filter(group => 
        group.activity && filters.activityLevels.includes(group.activity.level)
      );
    }
    ...
    if (filters.minRevenue !== undefined) {
      result = result.filter(group => 
        group.metrics && group.metrics.monthlyRevenue >= filters.minRevenue!
      );
    }
    if (filters.maxRevenue !== undefined) {
      result = result.filter(group => 
        group.metrics && group.metrics.monthlyRevenue <= filters.maxRevenue!
      );
    }
