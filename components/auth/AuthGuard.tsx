...
interface AuthGuardProps {
  ...
  requiredRole?: 'platform_admin' | 'restaurant_admin' | 'supplier_admin' | 'admin' | 'restaurant' | 'supplier'
...
export function AuthGuard({ 
  ...
  }, [isAuthenticated, user, allowSuperUser, canViewAsOrganization, viewMode, getCurrentRole, requiredRole])
          ...
          <CardContent className="p-6 text-center">
              ...
              {viewMode.isViewingAs && (
                <div className="mt-2 p-2 bg-orange-100 rounded border border-orange-200">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-orange-800 font-medium">檢視模式</span>
                  </div>
            ...
            <div className="space-y-2">
              {viewMode.isViewingAs ? (
                ...
                <Button 
                  onClick={() => router.push(user?.role?.startsWith('restaurant_') ? '/restaurant' : 
                                             user?.role?.startsWith('supplier_') ? '/supplier' : 
