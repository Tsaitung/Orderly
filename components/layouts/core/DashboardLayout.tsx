...
function DemoModeBanner({ currentRole }: DemoModeBannerProps) {
  ...
  return (
    <div className={cn(
      "text-white text-center py-2.5 text-sm relative z-50",
      ...
      {toggleLink && (
        <Link 
          ...
          className="ml-4 text-xs opacity-75 hover:opacity-100 hover:underline transition-all cursor-pointer"
        >
          {toggleLink.text} →
        </Link>
      )}
      ...
      <div className="flex">
        ...
        <div className="flex-1 lg:ml-60 flex flex-col">
          ...
          <div className="sticky top-0 z-40">
            {showDemoMode && (
              <DemoModeBanner currentRole={role} />
            )}
            <DashboardHeader
              title={title}
              subtitle={subtitle}
...
export { useDashboard, useDashboardTheme, useDashboardSpacing, useDashboardNavigation } from './DashboardProvider'