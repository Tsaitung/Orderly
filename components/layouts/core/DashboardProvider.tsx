...
export function DashboardProvider({
  ...
  return (
    <DashboardContext.Provider value={contextValue}>
      <div className={`min-h-screen transition-colors duration-200 theme-${role}`}>
        {children}
      </div>
...
export function useDashboardNavigation() {
  ...
  const getNavigationItemClasses = (item: NavigationItem, currentPath: string) => {
    const baseClasses = 'flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors min-h-[44px]'
