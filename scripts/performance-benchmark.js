...
const log = {
  ...
  metric: (label, value, unit = '') => console.log(`  ${colors.cyan}${label}:${colors.reset} ${colors.bold}${value}${unit}${colors.reset}`)
};
...
async function runStabilityTest(url, description, duration = 60000) {
  ...
  while (performance.now() - startTime < duration) {
    ...
    if (requestCount % 10 === 0) {
      ...
      process.stdout.write(`\r  進度: ${requestCount} 請求, ${elapsed.toFixed(1)}s, QPS: ${qps.toFixed(2)}`);
    }
...
async function runBenchmarks() {
  ...
  for (const endpoint of testEndpoints.slice(0, 2)) { // 只測試統計端點的穩定性
