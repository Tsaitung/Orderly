...
async function testRestaurantRegistration() {
  ...
  try {
    const response = await apiCall('POST', `${USER_SERVICE_URL}/auth/register`, TEST_RESTAURANT)
...
async function testSupplierOnboarding(invitationCode) {
  ...
  try {
    ...
    const response = await apiCall('POST', `${USER_SERVICE_URL}/invitations/accept`, onboardingData)
