...
async function runSingleTest(testName, apiGatewayUrl, directUrl) {
  ...
  const performanceChange = (
    ((gatewayResult.responseTime - directResult.responseTime) / directResult.responseTime) *
    100
  ).toFixed(1)

  log.success(`結構完全匹配`)
  log.info(
    `性能比較: Gateway(${gatewayResult.responseTime}ms) vs Direct(${directResult.responseTime}ms)`
  )
