 ...
 */
export function useSupplierCustomers(organizationId?: string, initialParams: CustomerFilterParams = {}) {
  ...
  const bulkUpdateCustomers = useAsyncMutation(
    ({ customerIds, data }: { customerIds: string[]; data: Partial<SupplierCustomerCreateRequest> }) =>
  ...
  const exportCustomers = useCallback(async (format: 'csv' | 'xlsx' = 'csv') => {
    try {
...
export function useFileUpload() {
  ...
  const uploadFile = useCallback(async (file: File, type: 'document' | 'image' = 'document') => {
...
export function useSupplierData(organizationId?: string) {
  ...
  const isLoading = profile.loading || dashboard.loading || customers.loading || 
                   onboarding.loading || orders.loading || products.loading || finance.loading;
  const hasError = !!(profile.error || dashboard.error || customers.error || 
                     onboarding.error || orders.error || products.error || finance.error);
  ...
  }, [profile.refetch, dashboard.refreshMetrics, customers.refetch, onboarding.refetch, 
      orders.refetch, products.refetch, finance.refetch]);
