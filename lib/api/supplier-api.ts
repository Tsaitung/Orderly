...
async function apiRequest<T>(url: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new SupplierApiError('Network error occurred', 0, 'NETWORK_ERROR')
  }
}
...
export const supplierInvitationApi = {
  ...
  async sendInvitation(request: InvitationSendRequest): Promise<InvitationSendResponse> {
    return apiRequest<InvitationSendResponse>(`${USER_SERVICE_URL}/invitations/send`, {
      method: 'POST',
  ...
  async cancelInvitation(invitationId: string): Promise<{ message: string }> {
    return apiRequest(`${USER_SERVICE_URL}/invitations/${invitationId}`, { method: 'DELETE' })
  },
}
...
export const supplierOnboardingApi = {
  ...
  async completeOnboarding(
    ...
    return apiRequest<SupplierOnboardingResponse>(`${USER_SERVICE_URL}/invitations/accept`, {
      method: 'POST',
      body: JSON.stringify(request),
    })
  },
  ...
  async getProfile(): Promise<OrganizationResponse> {
    return apiRequest<OrganizationResponse>(`${USER_SERVICE_URL}/invitations/profile`)
  },
...
export const supplierProductApi = {
  ...
  async createSKU(skuData: any): Promise<any> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/skus`, {
      method: 'POST',
  ...
  async updateSKU(skuId: string, skuData: any): Promise<any> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/skus/${skuId}`, {
      method: 'PUT',
      body: JSON.stringify(skuData),
    })
  },
  ...
  async deleteSKU(skuId: string): Promise<{ message: string }> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/skus/${skuId}`, { method: 'DELETE' })
  },
  ...
  async bulkSKUOperation(operation: string, skuIds: string[]): Promise<any> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/skus/bulk`, {
      method: 'POST',
  ...
  async importSKUs(file: File, options: any = {}): Promise<any> {
    ...
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/skus/import`, {
      method: 'POST',
  ...
  async exportSKUs(filters: any = {}, format: 'csv' | 'excel' = 'csv'): Promise<Blob> {
    ...
    const response = await fetch(`${PRODUCT_SERVICE_URL}/v1/skus/export?${params.toString()}`, {
      headers: createAuthHeaders(),
    })

    if (!response.ok) {
      throw new SupplierApiError('Export failed', response.status, 'EXPORT_ERROR')
    }
...
export const supplierPricingApi = {
  ...
  async updatePricingRule(ruleId: string, ruleData: any): Promise<any> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/pricing/rules/${ruleId}`, {
      method: 'PUT',
      body: JSON.stringify(ruleData),
    })
  },
  ...
  async deletePricingRule(ruleId: string): Promise<{ message: string }> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/pricing/rules/${ruleId}`, { method: 'DELETE' })
  },
  ...
  async calculatePricing(skuIds: string[], customerTier?: string): Promise<any> {
    return apiRequest(`${PRODUCT_SERVICE_URL}/v1/pricing/calculate`, {
      method: 'POST',
