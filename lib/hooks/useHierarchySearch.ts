...
export function useHierarchySearch(
  ...
  const saveRecentSearches_fn = useCallback((searches: string[]) => {
    if (saveRecentSearches && typeof window !== 'undefined') {
      try {
  ...
  const performSearch = useCallback(async (query: string, filters: SearchFilters = {}) => {
    ...
    if (trimmedQuery && !recentSearchesRef.current.includes(trimmedQuery)) {
      const newRecentSearches = [trimmedQuery, ...recentSearchesRef.current]
  ...
  const searchRecent = useCallback(async (recentQuery: string): Promise<void> => {
    await search(recentQuery);
  }, [search]);

  // Load suggestions (removed circular dependency)
  const loadSuggestions = useCallback(async (partial: string): Promise<void> => {
    ...
    try {
      ...
      if (suggestionSources.includes('autocomplete')) {
        ...
        const commonTerms = [
          '餐廳', '公司', '地點', '廚房', '總公司', '分店', '營業處', '業務部',
          'restaurant', 'company', 'location', 'kitchen', 'headquarters', 'branch'
        ];
