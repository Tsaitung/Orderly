...
export function useHierarchyNavigation(
  ...
  const updateUrl = useCallback((nodeId: UUID, nodeType: HierarchyNodeType) => {
  ...
  const loadNodeData = useCallback(async (nodeId: UUID, nodeType: HierarchyNodeType): Promise<void> => {
    try {
  ...
  const addToHistory = useCallback((nodeId: UUID, nodeType: HierarchyNodeType, nodeName: string) => {
  ...
  const navigateToNode = useCallback(async (nodeId: UUID, nodeType: HierarchyNodeType): Promise<void> => {
  ...
  const navigateToPath = useCallback(async (path: string): Promise<void> => {
  ...
  const navigateToHistoryItem = useCallback(async (item: NavigationHistory): Promise<void> => {
  ...
  const navigateToChild = useCallback(async (childId: UUID): Promise<void> => {
  ...
  const navigateToSibling = useCallback(async (siblingId: UUID): Promise<void> => {
  ...
  const canNavigateBack = useMemo(() => historyIndex < history.length - 1, [historyIndex, history.length]);
