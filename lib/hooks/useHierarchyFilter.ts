...
export function useHierarchyFilter(
  ...
  const addLocation = useCallback((type: 'country' | 'city' | 'region', value: string) => {
    setState(prev => {
      const currentValues = prev[type === 'country' ? 'countries' : type === 'city' ? 'cities' : 'regions'];
      return {
        ...prev,
  ...
  const setParentFilter = useCallback((parentId: UUID | null, parentType: HierarchyNodeType | null) => {
  ...
  // Preset management
  const saveAsPreset = useCallback((name: string, description?: string, isPublic: boolean = false): FilterPreset => {
  ...
  const validateCurrentFilters = useCallback((): { isValid: boolean; errors: string[] } => {
    ...
    if (state.creditLimitRange && state.creditLimitRange.min && state.creditLimitRange.max && 
    ...
    if (state.budgetRange && state.budgetRange.min && state.budgetRange.max && 
        state.budgetRange.min > state.budgetRange.max) {
      errors.push('Budget range is invalid: minimum must be less than maximum');
    }

    if (state.hierarchyDepth && state.hierarchyDepth.min && state.hierarchyDepth.max && 
