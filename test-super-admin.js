...
async function testSuperAdminEndpoints() {
  ...
  try {
    ...
    try {
      ...
      console.log('🔄 Service may not be running. Try starting with: npm run dev -w backend/user-service');
      return;
    }
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (error.response?.status === 404) {
          ...
          console.log(`✅ ${endpoint.description} is available (responded with ${error.response?.status})`);
        }
      }
    }
