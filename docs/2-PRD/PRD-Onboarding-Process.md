...
> 📖 **完整 User Story**：[US-ONB-001 至 US-ONB-011](../1-User-Story/by-module/07-onboarding-process.md)
...
- 邀請機制：
  ...
  - 可輸入既有供應商資訊邀請加入
- 合作夥伴邀請導向（雙向）：
  - 若餐廳是由供應商邀請連結/QR Code 進入：預設帶入邀請方供應商並加入常用（Favorite），仍可額外選擇/邀請其他供應商
  - 若供應商是由餐廳邀請連結/QR Code 進入：完成接受邀請後，餐廳端供應商池自動新增該供應商並標記 Favorite（可取消）
...
- 邀請分享方式（Phase 1）：
  - Line 分享（含 QR Code）
  - 邀請 Email（可選；可僅分享連結/QR）
- 受邀落地後需引導餐廳選擇「既有合作夥伴 / 新合作夥伴」，並在建立關係後預設互相標記 Favorite（可取消）
