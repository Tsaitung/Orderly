...
- 商品目錄與 SKU（最小可用）：分類、商品、SKU、價格/稅務狀態（供下單引用）
- 簡化訂單流程：僅支援核心狀態流轉
- 價格表管理：支援固定價格和時價
- 基礎驗收功能：數量確認和異常標記
- 多租戶架構：餐廳/供應商角色隔離
...
> **依賴關係（開發順序）**：訂單與對帳都必須引用已存在的商品/SKU，因此實作上需先完成「商品目錄與 SKU 管理系統」的最小可用，再進入訂單、驗收與對帳流程。
...
> 📖 **完整 User Story**：[US-BIL-001 至 US-BIL-008](../1-User-Story/by-module/05-billing-settlement.md)
...
> 📖 **完整 User Story**：[US-ORD-001 至 US-ORD-007](../1-User-Story/by-module/03-order-management.md)
...
> 📖 **完整 User Story**：[US-ACC-001 至 US-ACC-007](../1-User-Story/by-module/04-acceptance-receiving.md)
...
> 📖 **完整 User Story**：[US-PRD-001 至 US-PRD-020](../1-User-Story/by-module/02-product-sku-management.md)
>
> **新增功能情境**（2025-12-18）：
> - US-PRD-011~013：多供應商同一 SKU 名稱整合（別名管理、命名規則）
> - US-PRD-014~016：SKU 圖片統一與審核機制（上傳審核、主圖片、預設圖）
> - US-PRD-017~020：品牌與子品牌欄位設計（有品牌/無品牌商品支援）
...
| 收藏/常用（Favorite） |   ✅   |   ✅   |   👁️   | 星號標記、置頂排序；接受邀請後預設互相收藏，可手動取消 |
...
- [ ] 邀請連結預設30天有效（可設定）
- [ ] 支援批量邀請（單次<100個）
- [ ] 接受邀請時需選擇「既有合作夥伴 / 新合作夥伴」，並可填既有對照編號（選填）
- [ ] 建立關係後預設互相標記 Favorite，且可在供應商/餐廳名單中快速篩選/置頂
- [ ] 合作狀態變更即時通知
- [ ] 績效數據每日更新
- [ ] 支援合作協議檔案上傳
- [ ] 智能匹配準確率>80%
...
- [UI/UX 設計規範](../0-Design/design-system/INDEX.md)
- [API 開發規範（精要）](../0-Design/API-Endpoints-Essential.md)
- [API 規格（OpenAPI 3）](../0-Design/api-specification.yaml)
- [資料庫設計規範](../0-Design/database.md)
- [安全與機敏配置（Secrets/Config）](../3-Development-Plan/ci-secrets.md)
- [ERP 整合指南](../0-Design/erp-integration-guide.md)
- [技術架構文檔](../0-Design/Technical-Architecture-Summary.md)
