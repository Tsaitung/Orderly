    ...
    <Card className="p-4 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 text-xs">
        <div className="flex items-center justify-between">
          ...
          <span className="text-gray-600 truncate ml-2">{sku.base_unit}</span>
        </div>
        
        {/* 規格資訊 */}
        <div className="text-gray-500 line-clamp-2">
          {formatSpecification(sku.specification)}
        </div>
      </div>
...
const getStockStatusLabel = (status: string) => {
  switch (status) {
    case 'in_stock': return '庫存充足';
    case 'low_stock': return '庫存偏低';
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="text-right">
                    <div className="text-lg font-semibold">
                      NT$ {participation.selling_price}
                    </div>
                    <div className="text-sm text-gray-500">
                      / {sku.base_unit}
                    </div>
        ...
        {step === 'search' && (
          <div className="space-y-4">
            ...
            {suggestedSKUs.length > 0 && (
              <div>
                <h4 className="font-medium mb-2">發現相似的共享商品：</h4>
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {suggestedSKUs.map((sku) => (
                    <Card key={sku.id} className="p-3 hover:bg-gray-50 cursor-pointer">
                      <div className="flex items-center justify-between">
                        <div>
                        ...
                        <Button
                          size="sm"
                          onClick={() => handleJoinExistingSKU(sku.id)}
                        >
                          加入販售
                        </Button>
                      </div>
