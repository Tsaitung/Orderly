...
| INV-auth-001 | 所有資料查詢強制帶 tenant_id；缺則拒絕或回 401/403（impersonation 為明文例外，見 INV-auth-003：effective tenant 仍滿足「查詢帶 tenant_id」）| ∀ query：tenant_id IS NOT NULL（租戶隔離）；impersonation 下 tenant_id = effective(目標)租戶 | auth / 全模組 | repository 層強制注入 tenant_id；整合測試驗跨租戶不可讀 | `CLAUDE.md` §Auth/User 整合風險與緩解（租戶隔離）+ §使用者資料模型 | confirmed |
...
| INV-auth-003 | 超管模擬登入（impersonation / act-as）雙 context：effective `tenant_id` = 目標帳號租戶且授權用目標 role/permissions（不繼承 `super_user` override/bypass）；token 與每筆 audit 同時記錄真實 actor（super_admin）與被模擬 user | impersonation session：authz(role,permissions,tenant) == target；audit ∋ {actor_id, impersonated_user_id, tenant_id}；不存在 act_as 放寬 permissions 的路徑 | auth | 整合測試：act-as 受目標角色約束（越權回 403）、跨租戶隔離（讀不到他租戶）、每筆 audit 含雙 id | `docs/1-User-Story/by-module/01-auth-user-management.md`（US-AUTH-023）+ `docs/2-PRD/PRD-Auth-Module.md` §2.5 + `docs/0-Design/technical-architecture-auth.md` §10.3 + [ADR-0005](adr/ADR-0005-auth-super-admin-impersonation.md) | confirmed |
