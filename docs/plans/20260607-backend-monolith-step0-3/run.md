...
- **STEP 3**：寫 composition root `backend/app/main.py`，`include_router` 全部 router 於 surveyed 前綴（保留 dual-prefix + gateway 無前綴 alias + 雙前綴 `/api/api/suppliers`），SecurityHeaders + Auth + Redis rate-limit middleware 套一次於 top-level，serve `/health`；各模組保留自己 engine/Base；建 `backend/app/requirements.txt`（聯集）與 `.claude/restart.yaml`。
...
- **新檔**：`backend/app/main.py`、`backend/app/__init__.py`、`backend/app/requirements.txt`、`backend/libs/orderly_fastapi_core/pyproject.toml`、`.claude/restart.yaml` ≈ 5 個真正新檔。
...
| 路徑 | 動作 | 職責 |
|------|------|------|
| `backend/libs/orderly_fastapi_core/pyproject.toml` | Create | 讓共用核心可 `pip install -e`（解 sys.path hack 根因）|
...
| `backend/app/modules/gateway_compat/**` | Move | = 原 `api-gateway-fastapi`（本階段只取 middleware + 無前綴 alias；proxy 邏輯 STEP 4 才退役）|
...
**Files:**
- Create: `backend/libs/orderly_fastapi_core/pyproject.toml`
- Modify: 5 個用 `from libs.orderly_fastapi_core` 的檔（grep 定位）+ 含 `sys.path.append/insert` 的檔

- [ ] **T0.1 確認核心無 packaging**：`ls backend/libs/orderly_fastapi_core/{pyproject.toml,setup.py,setup.cfg}` — 預期皆無 → 需新增。
- [ ] **T0.2 寫 pyproject.toml**：
...
where = ["."]
include = ["orderly_fastapi_core*"]
```
（若 lib 目錄名非 `orderly_fastapi_core` 套件結構，調整 `package-dir`；以實際 `ls` 為準。）

- [ ] **T0.3 editable 安裝**：`direnv exec . pip install -e backend/libs/orderly_fastapi_core`。預期：`Successfully installed orderly-fastapi-core-0.1.0`。
...
- [ ] **T0.7 Commit**：`git add backend/libs/orderly_fastapi_core/pyproject.toml <改動的檔>` → `git commit -m "refactor(core): make orderly_fastapi_core pip-installable; drop sys.path hacks + normalize imports"`。
...
- [ ] **T1b.1 確認 dead**：`grep -rn "from app.models.sku import\|from .sku import\|models\.sku\b\|import app.models.sku" backend/product-service-fastapi --include=*.py | grep -v __pycache__ | grep -vE "sku_simple|sku_upload|supplier_sku"`。預期：0 load-bearing hit（`models/__init__.py` 已證實從 `.sku_simple`/`.supplier_sku` import，非 `sku.py`）。
- [ ] **T1b.2** 若 T1b.1 為 0 → `git rm backend/product-service-fastapi/app/models/sku.py`。若非 0 → STOP，回報（claim 不成立，需重評）。
- [ ] **T1b.3 驗證**：`cd backend/product-service-fastapi && direnv exec . python -c 'import app.models'` 無 `InvalidRequestError`（重複 mapper 消失）。
- [ ] **T1b.4 Commit**：`git commit -m "fix(product-service): delete dead duplicate-mapper models/sku.py (ProductSKU/SupplierSKU shadow)"`。
...
| 9 | `gateway_compat` | `api-gateway-fastapi` | 最後；本階段只保留 middleware + 無前綴 alias，proxy 邏輯留到 STEP 4 |
...
| WebSocket | `ws://localhost:$BACKEND_PORT/ws/orders/<orgId>` | 連線成功 |
...
| R4 | composition root 掛載後路由前綴碰撞（/health、/auth/login 多源）| high | gateway_compat 只留 alias，user-service router 為單一 owner；T3.5 contract 清單逐條驗 |
...
| D-gw | gateway 的 Auth/rate-limit 此階段如何處理 | **移植成 top-level middleware**（不刪 gateway_compat 模組本身，proxy 邏輯留 STEP 4 退役）| STEP 3 middleware |
...
- **AC3**：單一 `uvicorn app.main:app` 起；§STEP 3 frozen contract 表 8 條全綠（含 login JSON 形狀、雙前綴、/api/v2、/ws/orders）。
