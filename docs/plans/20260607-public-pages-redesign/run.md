...
| 修改既有檔 | 6 | app/page.tsx, app/layout.tsx, tailwind.config.js, HeroSection.tsx（移除前過渡）, RoleSelector.tsx（移除前過渡）, NavigationHeader.tsx |
...
| `components/landing/LandingFooter.tsx` | Create | 四欄 nav + 法務 + 聯絡 + 繁中；取代 app/page.tsx 舊 footer |
...
| `tailwind.config.js` | Modify | `darkMode: 'class'`；確認 mocha/accent/status token、字體 family |
...
驗收：每個命名目標 commit 後 `grep -rn '<字串>' app/ components/` 在公開頁範圍內 0 hit（SystemStatus 元件檔本身保留，只驗 app/page.tsx 不再 import）。
...
- [ ] Step 2: `tailwind.config.js` 加 `darkMode: 'class'`（在 module.exports 頂層）；確認 `fontFamily` 含 Noto Sans TC / Inter / JetBrains Mono
...
- [ ] Step 5: `npm run type-check` 通過；`npm run dev` 起得來
...
- [ ] Step 2: 驗 `grep -rn '大樂司\|樂多多\|烤食組合\|稻舍' app/ components/` → 0 hit
...
- [ ] Step 1: 寫 Playwright spec（先 RED，元件未建會 fail）涵蓋：
  - `/` 載入，hero 標題含「8 小時」「30 分鐘」
  - 12 區塊各有可見錨點 id（#features #pricing #reconciliation #roles #faq 等）
  - **每個 nav/CTA 按鈕可點且導到存在的目標**（錨點捲動到對應 section；/login /register /contact 回 200）
...
- [ ] Step 2: Run `npx playwright test e2e/public-pages.spec.ts`，Expected: PASS（全綠）
...
- [ ] Step 1: `/api/contact` route：POST 驗證必填（身分/公司/聯絡人/email/需求）→ `console.log` → 回 `{ ok: true }`（正式寄信 Out of Scope）
...
- [ ] Step 2: `npm run type-check` + `npm run lint` 通過（輸出存 /tmp 完整讀，不截斷）
- [ ] Step 3: dev server 真實點擊全部按鈕一遍，確認零死連結（每個按鈕導真路由或捲到錨點）
- [ ] Step 4: 命名目標總驗：`grep -rn '大樂司\|樂多多\|烤食組合\|稻舍\|demo1234\|demo.orderly.tw\|images.unsplash.com' app/ components/` → 0 hit
...
- AC6（型別/lint）：`npm run type-check` + `npm run lint` 綠（完整輸出，不截斷）。
