...
US-AUTH-023/024 (US) ── PRD §2.5 ── tech-arch §10.3 ── ADR-0005 / INV-auth-003
 ...
 backend/app/modules/users (auth)     shared/types (impersonation DTO / act_as claim)
        │                                   │
        ▼                                   ▼
 gateway_compat/middleware (auth context)  frontend（清單入口 + 橫幅 + role switch）
```
...
| `backend/app/modules/users/api/v1/` | Create/Modify | impersonation router（start/stop/current/role-switch）|
| `backend/app/modules/gateway_compat/middleware/` | Modify | auth context 透傳；effective-context authorize（**先讀現有 middleware 確認 context 傳遞機制**，見 M3 註）|
...
- token：沿用既有 JWT 機制，新增 `act_as` claim block。**effective identity 契約**：token/context 同時保留 `actor_user_id`(super_admin) 與 `effective_user_id`(=impersonated)；**業務授權與 current-user 一律用 effective target**（`role`/`permissions`/`organization_id`），**audit 用 actor+target**，**禁止繼承 actor 的 `is_super_user` override**（不破 INV-auth-001 / INV-auth-003）
...
2. **Backend auth**：新 `impersonation_service.py` + api/v1 router（start/stop/current + role-switch）；session 走既有 `SessionService`；token 簽發帶 `act_as`；guard（super_admin + MFA-passed + target active）
3. **Backend authz/middleware**：先讀 `gateway_compat/middleware/` 確認既有 auth context 傳遞機制，再在其上讓下游 authorize 用 effective `role`/`permissions`/`organization_id`；確保 INV-auth-001 仍成立
