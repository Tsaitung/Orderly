...
> **登入模型變更（2026-06-08）**：平台僅提供 **Line 登入（主要）** 與 **Google 登入（次要，需先綁定）** 兩種方式，適用全體使用者（含平台內部員工）。**不再提供 Email + 密碼登入**，密碼於認證流程中完全廢除。**Email 不再用於登入或帳號恢復**，僅作財務對帳聯絡用途。帳號恢復依賴另一個已綁定的社群帳號（Line 失效時用 Google）；兩者皆失效須走人工支援。MFA（TOTP / SMS）可作為第二因素疊加於社群登入之上。詳見 [PRD-Auth-Module.md](../../2-PRD/PRD-Auth-Module.md) Section 4.1.1、4.5。
...
**驗收標準**:
- [ ] **Line 快速註冊（唯一註冊路徑）**：一鍵授權，自動取得 Line 顯示名稱與頭像
- [ ] 不再提供 Email + 密碼註冊（平台僅支援 Line 主要 / Google 次要）
- [ ] 必填：公司名稱、統編、電話
- [ ] Email 為**選填**，且**僅作財務對帳聯絡用途**（非帳號登入或恢復）
- [ ] 電話 SMS OTP 驗證（6 碼，10 分鐘有效）
- [ ] 註冊完成後於系統內顯示下一步指引
...
- [ ] **Google Login（次要方式）**：已綁定 Google 的帳號可用 Google 登入；登入頁顯示「使用 Google 登入」作為次要入口
- [ ] **不再提供 Email + 密碼登入**（其他登入方式一律移除）
- [ ] 登入響應時間 <2 秒
...
**優先級**: P0
**狀態**: Deprecated（2026-06-08）

> **已停用原因**：登入模型變更後密碼於認證流程中完全廢除（僅 Line/Google 社群登入），無密碼可重設。帳號恢復改由 **US-AUTH-022 社群帳號恢復** 處理。

**故事（歷史）**: 作為餐廳操作員，我希望能安全地重設忘記的密碼，以便恢復帳戶存取

**驗收標準（已不適用）**:
- [x] ~~透過 CAPTCHA 防止濫用~~
- [x] ~~寄送 1 小時有效的單次使用重設連結~~
- [x] ~~重設後強制登出所有 Session~~
- [x] ~~完成後發送通知 Email~~
- [x] ~~每日最多 3 次重設嘗試~~
...
**驗收標準**:
- [ ] Level 1（Line 帳號建立）：可瀏覽/收藏，限 3 筆試用訂單
- [ ] Level 2（電話 SMS 驗證）：可下正式單（限額）、基本分析
- [ ] Level 3（營業驗證）：完整功能、無訂單限制、所有付款方式
- [ ] 驗證進度清楚顯示

> 註：原 Level 1「Email 驗證」改為「Line 帳號建立」；Email 不再作認證用途，僅為財務對帳選填欄位。

**來源**: [PRD-Auth-Module.md](../../2-PRD/PRD-Auth-Module.md) Section 6

---

### US-AUTH-006: Line 主要登入與 Google 次要綁定
**角色**: restaurant_operator（餐廳操作員）
**優先級**: P0
**狀態**: Active

**故事**: 作為餐廳操作員，我希望能使用 Line 帳號一鍵登入並可綁定 Google 作為次要登入，以便快速進入系統同時保有備援登入管道

**驗收標準**:
- [ ] **Line Login 為主要登入方式**，登入頁首選顯示
- [ ] 首次 Line 登入即建立帳號，**不再強制綁定 Email**
- [ ] 支援在設定頁綁定 **Google 帳號作為次要登入方式**（綁定後可用 Google 登入）
- [ ] 支援 Line 登入後自動同步頭像與顯示名稱
- [ ] Line 帳號與平台帳號一對一綁定；Google 帳號可額外綁定同一平台帳戶
- [ ] 支援解除社群綁定，但**帳號必須至少保留一個有效社群綁定（Line 或 Google）**，不得解除至零
- [ ] Email 僅為選填的財務對帳聯絡資訊，與登入/綁定無關
...
- [ ] 若未登入：以 **Line 登入/註冊** 為主路徑，**Google 登入為次要**（不再提供 Email 備選登入）
- [ ] 若已有帳號：登入後可選擇將邀請綁定至目前登入的組織（需二次確認避免誤綁）
- [ ] 受邀註冊/加入後仍走既有安全流程（電話 SMS OTP 驗證、必要商業資訊），不得因邀請跳過；不再要求 Email 綁定
...
### US-AUTH-022: 社群帳號恢復（取代密碼重設）
**角色**: restaurant_operator（餐廳操作員）、supplier_operator（供應商操作員）
**優先級**: P0
**狀態**: Active（本次新增）

**故事**: 作為餐廳或供應商操作員，當我的主要 Line 帳號無法使用時，我希望能用已綁定的 Google 帳號恢復存取，以便不必依賴密碼或 Email 即可回到系統

**驗收標準**:
- [ ] 主要登入（Line）不可用時，可改用已綁定的 **Google 次要登入**直接進入帳號
- [ ] 設定頁鼓勵使用者綁定第二個社群帳號（Google）作為恢復保險
- [ ] 帳號至少保留一個有效社群綁定（與 US-AUTH-006 一致）
- [ ] 兩個社群帳號皆失效時，提供**人工支援恢復**流程：身分驗證（統編/營業資訊/已知交易）後由 platform_support 協助重新綁定
- [ ] 不使用 Email 連結或密碼重設作為恢復手段
- [ ] 所有恢復行為記錄完整審計日誌（actor、entity、方式、結果）
...
**驗收標準**:
- [ ] 支援 SMS OTP、TOTP（Authenticator App）作為第二因素；**不再使用 Email OTP**（Email 僅作財務對帳用途）
- [ ] supplier_admin 強制啟用 MFA
- [ ] 提供備用碼功能
- [ ] MFA 設定變更需重新驗證身份（透過社群重新授權或現有 MFA）

**來源**: [PRD-Auth-Module.md](../../2-PRD/PRD-Auth-Module.md) Section 4.2

---

### US-AUTH-008: Google 綁定與次要登入
**角色**: restaurant_operator（餐廳操作員）
**優先級**: P1
**狀態**: Active

**故事**: 作為餐廳操作員，我希望能將 Google 帳號綁定到我的平台帳號並作為次要登入方式，以便在 Line 不可用時仍能登入

**驗收標準**:
- [ ] 支援 Google OAuth 作為**次要登入方式**（綁定後可用 Google 一鍵登入）
- [ ] Google 綁定流程：登入後於設定頁授權 Google → 確認綁定至目前帳號
- [ ] 綁定既有帳戶需使用者確認，避免誤綁
- [ ] OAuth 登入後若帳戶啟用 MFA 仍需驗證
- [ ] Google 帳號可與 Line 帳號同時綁定同一平台帳戶
- [ ] **不再要求綁定 Email**（Email 與登入/綁定無關，僅財務對帳用途）
...
- [ ] 支援 Android 指紋辨識
- [ ] 生物辨識失敗後回退至 Line Login 或 Google Login（不再回退至密碼登入）
- [ ] 可在設定中開關生物辨識
...
### US-AUTH-016: 平台用戶登入（Line/Google + 強制 MFA）
**角色**: platform_admin（平台管理員）、platform_support（平台支援）、super_admin（系統超管）
**優先級**: P0
**狀態**: Active

> **變更（2026-06-08）**：依登入模型統一決策，平台用戶改用與餐廳/供應商相同的 **Line 主要 / Google 次要** 社群登入，**移除原 Email + 密碼獨立登入系統**。安全等級改以「強制 MFA + 帳號限定供裝 + 更嚴格鎖定 + 完整審計 + IP 白名單」維持。

**故事**: 作為平台管理員，我希望能以 Line/Google 登入並通過強制 MFA，以便在統一登入模型下維護平台運營安全

**驗收標準**:
- [ ] 登入方式：**Line 主要 / Google 次要**（與餐廳/供應商一致），不再使用 Email + 密碼
- [ ] **強制 MFA**（TOTP 優先，SMS 次之）疊加於社群登入之上
- [ ] 平台用戶帳號**僅能由 super_admin 或現有 platform_admin 供裝**（非公開註冊）；社群帳號須綁定到預先建立的平台帳號才能登入
- [ ] 首次登入強制完成 MFA 設定
- [ ] 登入失敗 3 次後鎖定 15 分鐘（比一般用戶更嚴格）
- [ ] 所有登入行為記錄完整審計日誌
- [ ] 支援 IP 白名單限制登入來源（可選）
- [ ] Session 併發數限制最多 3 個（比一般用戶更嚴格）

**設計原則與風險權衡**:
- 採統一社群登入以簡化身份系統，安全等級改由 MFA + 供裝控管 + 審計維持
- ⚠️ 取捨：相較原獨立系統，增加對 Line/Google 可用性與第三方釣魚風險的暴露；緩解措施為強制 MFA、IP 白名單、帳號供裝限定與異常告警（詳見 PRD 4.5 決策註記）
...
|------|:------:|:------:|:------:|------------|
| 註冊流程（僅 Line）| ✅ | ✅ | ❌ | US-AUTH-001, US-AUTH-002 |
| 標準登入（Line 主 / Google 次）| ✅ | ✅ | ✅ | US-AUTH-003 |
| ~~密碼重設~~（已停用）| ❌ | ❌ | ❌ | US-AUTH-004（Deprecated）|
| 社群帳號恢復 | ✅ | ✅ | ✅ | US-AUTH-022 |
| 驗證等級 | ✅ | ✅ | ❌ | US-AUTH-005 |
| **Line 登入 + Google 綁定** | ✅ | ✅ | ✅ | US-AUTH-006 |
| **合作夥伴邀請（供應商↔餐廳）** | ✅ | ✅ | ❌ | US-AUTH-019, US-AUTH-020, US-AUTH-021 |
| MFA 設定 | ⚙️ | ⚙️ | 🔒 | US-AUTH-007 |
| Google 登入（次要綁定）| ✅ | ✅ | ✅ | US-AUTH-008 |
...
| 使用者類型 | 主要登入方式 | 次要方式 | MFA 要求 | 行動裝置支援 |
|-----------|-------------|---------|---------|-------------|
| **餐廳端** (restaurant_*) | Line Login | Google（綁定後）| 建議（admin 強烈建議）| Web + 原生 App |
| **供應商端** (supplier_*) | Line Login | Google（綁定後）| 強制（admin）/ 建議（manager）| Web + 原生 App |
| **平台端** (platform_*, super_admin) | Line Login | Google（綁定後）| 🔒 強制（TOTP）+ 帳號供裝限定 | 僅 Web（管理後台）|

> 📌 **設計原則（2026-06-08 更新）**：全平台統一採 **Line 主要 / Google 次要** 登入，不再提供 Email + 密碼。平台端不再使用獨立登入系統，改以強制 MFA、帳號供裝限定、更嚴格鎖定與 IP 白名單維持安全等級。Email 僅作財務對帳用途，不用於登入或帳號恢復。
